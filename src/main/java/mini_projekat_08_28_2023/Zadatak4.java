...
//                Napišite program koji omogućava korisniku da menja vrednosti unutar komande dok se ne
//        zamene sve vrednosti u komandi. Komanda iz primera je komanda koja pokreće Locust skriptu za stres testiranje.
//        Ukoliko ste zainteresovani, preporučuje se da istražite alat Locust.
//
//                Komanda je obican String, a vrednosti koje se menjaju su reči napisane velikim slovima
//        i ispred svake od njih se nalazi znak $. Ako u skripti postoji barem jedan znak $,
//        to znači da još uvek postoje promenljive koje treba zameniti.
//
//                Program od korisnika traži da unese naziv promenljive za zamenu i
//        vrednost koja će biti postavljena umesto te promenljive u komandi.
...
String locustFile = "locust -f $LOCUSTFILE -u $NUMBER_OF_USERS -r $SPAWN_RATE -t $RUN_TIME --headless –host $HOST --html $HTML_FILE";
        System.out.println(locustFile);
boolean terminate = false;

while (!terminate) {
        System.out.println("Unesite naziv promenljive za zamenu:");
        String selectedStringValue = s.next();

        System.out.println("Unesite vrednost:");
String replaceValue =s.next();

locustFile= locustFile.replace(selectedStringValue, replaceValue);
        System.out.println("Komanda: " + locustFile);
        if (!locustFile.contains("$")) {
            terminate=true;
        }
